...
export default async function AdminUsersPage() {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-3xl font-bold tracking-tight text-foreground dark:text-white">Χρήστες</h2>
                <p className="text-text-tertiary dark:text-text-quaternary mt-2">Διαχείριση εγγεγραμμένων χρηστών και δικαιωμάτων.</p>
            </div>

            <Card className="rounded-3xl border-border-dim shadow-sm overflow-hidden">
                <CardHeader>
                    <CardTitle>Εγγεγραμμένοι Χρήστες (Τελευταίοι 50)</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-text-tertiary">
                            <thead className="text-xs text-text-tertiary bg-surface-dim uppercase">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {users.map(user => (
                                    <tr key={user.id} className="hover:bg-surface-dim/50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-foreground">{user.fullName || "N/A"}</td>
                                        <td className="px-6 py-4">{user.email}</td>
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 bg-muted text-text-secondary rounded-2xl text-xs font-bold">{user.role}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            {user.status === "BANNED" ? (
