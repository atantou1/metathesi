...
import { ChevronDown, X, Database, SlidersHorizontal, Award } from 'lucide-react'
...
function PanelContent({ title, specialtyName, data, division, specialty, onClose, isMobile }: { title: string; specialtyName: string; division: string; specialty: string; data: any; onClose: () => void; isMobile: boolean }) {
  ...
  return (
    <div className={`w-full bg-background/80 backdrop-blur-xl h-full shadow-soft flex flex-col ${isMobile ? 'border-t' : 'border-l'} border-border/40`}>
        
        <div className={`${isMobile ? 'p-4' : 'p-6 pt-24'} border-b border-border-dim bg-transparent shrink-0`}>
        ...
        <div className={`${isMobile ? 'p-4 space-y-4' : 'p-6 space-y-8'} overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-slate-300`}>
            
            <div className="flex flex-col gap-4">
                
                {/* Αριθμός Μεταθέσεων - Primary KPI */}
                <div className="bg-card p-5 rounded-4xl border border-border-dim shadow-soft hover:shadow-floating hover:-translate-y-0.5 hover:border-primary/30 transition-all flex justify-between items-center group h-24">
                    <div>
                        <p className="text-label group-hover:text-primary transition-colors mb-1">Αριθμος Μεταθεσεων</p>
                        <div className="flex items-baseline space-x-2">
                            <span className="text-3xl font-extrabold text-foreground tracking-tight">{data.successCount}</span>
                        </div>
                    </div>
                    <div className="text-right">
                        <span className="inline-flex px-3 py-1.5 bg-primary-soft text-primary text-caption uppercase tracking-wider font-bold rounded-2xl border border-primary/20">
                          ΕΤΟΣ {successYear || 2024}
                        </span>
                    </div>
                </div>

                {/* 2-Column Grid for Base Score & Demand */}
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-card p-5 rounded-4xl border border-border-dim shadow-soft hover:shadow-floating hover:-translate-y-0.5 hover:border-primary/20 transition-all group flex flex-col justify-between h-24">
                        <p className="text-label group-hover:text-primary transition-colors mb-2">
                          Βαση Μοριων {baseYear ? `(${baseYear})` : ''}
                        </p>
                        <div className="flex flex-col items-start gap-1.5 mt-auto">
                            <span className="text-2xl font-extrabold text-foreground tracking-tight">{data.baseScore?.toFixed(2) || '—'}</span>
                            {data.baseScoreDiff !== null && data.baseScoreDiff !== 0 && (
                              <span className={`text-[11px] font-bold flex items-center px-2 py-0.5 rounded-2xl border ${data.baseScoreDiff > 0 ? 'text-danger bg-danger-soft border-danger/20' : 'text-success bg-success-soft border-success/20'}`}>
                    ...
                    <div className="bg-card p-5 rounded-4xl border border-border-dim shadow-soft hover:shadow-floating hover:-translate-y-0.5 hover:border-primary/20 transition-all group flex flex-col justify-between h-24">
                        <p className="text-label group-hover:text-primary transition-colors mb-2">Ζητηση (1η Προτ.)</p>
                        <div className="flex flex-col items-start gap-1.5 mt-auto">
                            <span className="text-2xl font-extrabold text-foreground tracking-tight">{data.targeting1stCount}</span>
                            {data.targeting1stCountDiff !== 0 && (
                              <span className={`text-[11px] font-bold flex items-center px-2 py-0.5 rounded-2xl border ${data.targeting1stCountDiff > 0 ? 'text-success bg-success-soft border-success/20' : 'text-danger bg-danger-soft border-danger/20'}`}>
                ...
                {/* Quality of Competition (Minimal layout) */}
                <div>
                    <h3 className="text-sm font-bold text-foreground tracking-wide flex items-center gap-2 mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-card p-5 rounded-4xl border border-border-dim shadow-soft hover:shadow-floating hover:-translate-y-0.5 hover:border-primary/20 transition-all group flex flex-col justify-between h-24">
                            <span className="text-label mb-1 group-hover:text-primary transition-colors">Μ.Ο. ΕΠΙΤΥΧΟΝΤΩΝ</span>
                            <div className="mt-auto">
                                <span className="text-xl font-extrabold text-foreground">{data.avgScore?.toFixed(1) || '—'}</span>
                            </div>
                        </div>
                        <div className="bg-card p-5 rounded-4xl border border-border-dim shadow-soft hover:shadow-floating hover:-translate-y-0.5 hover:border-primary/20 transition-all group flex flex-col justify-between h-24">
                            <span className="text-label mb-1 group-hover:text-primary transition-colors">Μ.Ο. ΑΙΤΟΥΝΤΩΝ</span>
                            <div className="mt-auto">
                                <span className="text-xl font-extrabold text-foreground">{data.avgScoreApplicants?.toFixed(1) || '—'}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div className={`${isMobile ? 'p-3' : 'p-6'} border-t border-border-dim bg-transparent shrink-0`}>
            <Link 
                href={`/stats/zones/${encodeURIComponent(title)}?division=${encodeURIComponent(division)}&specialty=${encodeURIComponent(specialty)}`}
                className={`w-full flex items-center justify-center bg-primary hover:bg-primary-hover text-white px-8 ${isMobile ? 'py-2.5' : 'py-3.5'} rounded-2xl text-sm font-semibold transition-all shadow-floating active:scale-[0.98] cursor-pointer`}
            >
                {isMobile ? 'Αναλυτικά Στατιστικά' : 'Προβολή Αναλυτικών Στατιστικών'}
            </Link>
        </div>
    </div>
  )
    ...
    >
      ...
      <main style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
          ...
          {isMobile && (
            <div style={{
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-background/60 backdrop-blur-xl border border-border-dim rounded-full text-sm font-semibold text-foreground shadow-floating active:scale-95 transition-all cursor-pointer whitespace-nowrap"
              >
        ...
        {/* ── Desktop Side Panel (Right) ─────────────────────────────────── */}
          ...
          {isMobile && (
            <div 
              className={`fixed bottom-0 left-0 right-0 shadow-floating z-[9999] transform transition-transform duration-300 ease-out flex flex-col border-t border-border-dim rounded-t-[32px] ${selectedZone ? 'translate-y-0' : 'translate-y-full'}`}
              style={{ 
                height: '50vh', 
